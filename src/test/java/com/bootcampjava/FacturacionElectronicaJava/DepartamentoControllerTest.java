package com.bootcampjava.FacturacionElectronicaJava;

import com.bootcampjava.FacturacionElectronicaJava.controllers.DepartamentoController;
...
class DepartamentoControllerTest {
    ...
    void listadoDepartamentos_DebeDevolverListaYStatus200() throws Exception {
        ...
        when(servicio.todosDepartamentos()).thenReturn(listaFalsa);
        // WHEN & THEN
    ...
    void listadoDepartamentos_CuandoNoHayDatos_DebeRetornarListaVacia() throws Exception {
        // GIVEN
        when(servicio.todosDepartamentos()).thenReturn(List.of());
        // WHEN & THEN
        mockMvc.perform(get("/api/v1/departamentos"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$").isArray())
